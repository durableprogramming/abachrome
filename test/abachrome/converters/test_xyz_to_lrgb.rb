...
class TestXyzToLrgb < Minitest::Test
  def convert(x, y, z, alpha = 1.0)
    Abachrome::Converters::XyzToLrgb.convert(
      Abachrome::Color.new(Abachrome::ColorSpace.find(:xyz), [x, y, z], alpha)
    )
  end

  def test_returns_lrgb_color
    lrgb = convert(0.5, 0.5, 0.5)
    assert_kind_of Abachrome::Color, lrgb
    assert_equal :lrgb, lrgb.color_space.name
  end

  def test_white_point_maps_to_white
    assert_coordinates_equal [1, 1, 1], convert(0.95047, 1.0, 1.08883).coordinates
  end
  ...
  def test_inverts_lrgb_to_xyz
    [[0.2, 0.6, 0.9], [1, 1, 1], [0.5, 0.1, 0.7], [0, 0, 0]].each do |coords|
  ...
  def test_alpha_is_preserved
    assert_in_delta 0.8, convert(0.5, 0.5, 0.5, 0.8).alpha, 0.001
  end
