...
class ColorsTest < Minitest::Test
  ...
  def test_all_colors_hex_parsing
    @colors_data.each_value do |data|
      hex = data["hex"]
      next unless hex
  ...
  def test_all_colors_rgb_values
    @colors_data.each_value do |data|
      ...
      next unless rgb_string.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/)
  ...
  def test_all_colors_component_access
    @colors_data.each_value do |data|
      ...
      assert color.red.to_f.between?(0, 1)
      assert color.green.to_f.between?(0, 1)
      assert color.blue.to_f.between?(0, 1)
      assert color.lightness.to_f.between?(0, 1)
